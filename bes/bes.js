window.EntryScreenFixerWebGL = class EntryScreenFixerWebGL {
    ...
    fixEntries() {
        ...
        this.parent.info(`${entries}개의 벡터 요소와 ${pngEntries}개의 비트맵 요소가 수정되었습니다.`)
    }

    fix() {
        const resolution = [Math.ceil(screen.width), Math.ceil(screen.width * 9 / 16)]

        this.parent.info(`이미 설정되어있습니다: ${screen.width}x${screen.height}`)
    ...
    setScreenResolution(w, h) {
        this.parent.info(`렌더링 해상도 설정: ${w}x${h}`)
...
window.EntryScreenFixer = class EntryScreenFixer {
    ...
    fix() {
        if(this.Entry.options.useWebGL) {
            ...
            const resolution = [Math.ceil(screen.width), Math.ceil(screen.width * 9 / 16)]
            this.info(`기기 해상도 ${screen.width}x${screen.height} 감지됨`)
            this.setScreenResolution(...resolution)
            this.fixSVG()
        }
    ...
    fixSVG() {
        ...
        this.info(`${entries}개의 벡터 요소가 수정되었습니다.`)
    }

    setScreenResolution(w, h) {
        this.info(`렌더링 해상도를 ${w}x${h}로 설정중`)
